; ModuleID = 'bench/sqlite3.ll'
source_filename = "sqlite-amalgamation-3300100/sqlite3.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

%struct.sqlite3_value = type { %union.MemValue, i16, i8, i8, i32, ptr, ptr, i32, i32, ptr, ptr }
%union.MemValue = type { double }

; Function Attrs: nounwind uwtable
declare hidden void @sqlite3DbFreeNN(ptr noundef, ptr noundef) #0

; Function Attrs: nounwind uwtable
define hidden void @vdbeMemClear(ptr noundef %p) #0 {
entry:
  %p.addr = alloca ptr, align 8
  store ptr %p, ptr %p.addr, align 8
  %0 = load ptr, ptr %p.addr, align 8
  %flags = getelementptr inbounds nuw %struct.sqlite3_value, ptr %0, i32 0, i32 1
  %1 = load i16, ptr %flags, align 8
  %conv = zext i16 %1 to i32
  %and = and i32 %conv, 9216
  %cmp = icmp ne i32 %and, 0
  br i1 %cmp, label %if.then, label %if.end

if.then:                                          ; preds = %entry
  %2 = load ptr, ptr %p.addr, align 8
  call void @vdbeMemClearExternAndSetNull(ptr noundef %2)
  br label %if.end

if.end:                                           ; preds = %if.then, %entry
  %3 = load ptr, ptr %p.addr, align 8
  %szMalloc = getelementptr inbounds nuw %struct.sqlite3_value, ptr %3, i32 0, i32 7
  %4 = load i32, ptr %szMalloc, align 8
  %tobool = icmp ne i32 %4, 0
  br i1 %tobool, label %if.then2, label %if.end4

if.then2:                                         ; preds = %if.end
  %5 = load ptr, ptr %p.addr, align 8
  %db = getelementptr inbounds nuw %struct.sqlite3_value, ptr %5, i32 0, i32 9
  %6 = load ptr, ptr %db, align 8
  %7 = load ptr, ptr %p.addr, align 8
  %zMalloc = getelementptr inbounds nuw %struct.sqlite3_value, ptr %7, i32 0, i32 6
  %8 = load ptr, ptr %zMalloc, align 8
  call void @sqlite3DbFreeNN(ptr noundef %6, ptr noundef %8)
  %9 = load ptr, ptr %p.addr, align 8
  %szMalloc3 = getelementptr inbounds nuw %struct.sqlite3_value, ptr %9, i32 0, i32 7
  store i32 0, ptr %szMalloc3, align 8
  br label %if.end4

if.end4:                                          ; preds = %if.then2, %if.end
  %10 = load ptr, ptr %p.addr, align 8
  %z = getelementptr inbounds nuw %struct.sqlite3_value, ptr %10, i32 0, i32 5
  store ptr null, ptr %z, align 8
  ret void
}

; Function Attrs: nounwind uwtable
declare hidden void @vdbeMemClearExternAndSetNull(ptr noundef) #0

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

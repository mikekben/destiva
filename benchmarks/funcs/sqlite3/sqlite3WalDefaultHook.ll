; ModuleID = 'bench/sqlite3.ll'
source_filename = "sqlite-amalgamation-3300100/sqlite3.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

; Function Attrs: nounwind uwtable
define hidden i32 @sqlite3WalDefaultHook(ptr noundef %pClientData, ptr noundef %db, ptr noundef %zDb, i32 noundef %nFrame) #0 {
entry:
  %pClientData.addr = alloca ptr, align 8
  %db.addr = alloca ptr, align 8
  %zDb.addr = alloca ptr, align 8
  %nFrame.addr = alloca i32, align 4
  store ptr %pClientData, ptr %pClientData.addr, align 8
  store ptr %db, ptr %db.addr, align 8
  store ptr %zDb, ptr %zDb.addr, align 8
  store i32 %nFrame, ptr %nFrame.addr, align 4
  %0 = load i32, ptr %nFrame.addr, align 4
  %1 = load ptr, ptr %pClientData.addr, align 8
  %2 = ptrtoint ptr %1 to i64
  %conv = trunc i64 %2 to i32
  %cmp = icmp sge i32 %0, %conv
  br i1 %cmp, label %if.then, label %if.end

if.then:                                          ; preds = %entry
  call void @sqlite3BeginBenignMalloc()
  %3 = load ptr, ptr %db.addr, align 8
  %4 = load ptr, ptr %zDb.addr, align 8
  %call = call i32 @sqlite3_wal_checkpoint(ptr noundef %3, ptr noundef %4)
  call void @sqlite3EndBenignMalloc()
  br label %if.end

if.end:                                           ; preds = %if.then, %entry
  ret i32 0
}

; Function Attrs: nounwind uwtable
declare dso_local i32 @sqlite3_wal_checkpoint(ptr noundef, ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden void @sqlite3BeginBenignMalloc() #0

; Function Attrs: nounwind uwtable
declare hidden void @sqlite3EndBenignMalloc() #0

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

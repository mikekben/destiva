; ModuleID = 'bench/sqlite3.ll'
source_filename = "sqlite-amalgamation-3300100/sqlite3.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

%struct.Walker = type { ptr, ptr, ptr, ptr, i32, i8, %union.anon.14 }
%union.anon.14 = type { ptr }
%struct.IdxCover = type { ptr, i32 }

; Function Attrs: nocallback nofree nounwind willreturn memory(argmem: write)
declare void @llvm.memset.p0.i64(ptr writeonly captures(none), i8, i64, i1 immarg) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @sqlite3WalkExpr(ptr noundef, ptr noundef) #1

; Function Attrs: nounwind uwtable
define hidden i32 @sqlite3ExprCoveredByIndex(ptr noundef %pExpr, i32 noundef %iCur, ptr noundef %pIdx) #1 {
entry:
  %pExpr.addr = alloca ptr, align 8
  %iCur.addr = alloca i32, align 4
  %pIdx.addr = alloca ptr, align 8
  %w = alloca %struct.Walker, align 8
  %xcov = alloca %struct.IdxCover, align 8
  store ptr %pExpr, ptr %pExpr.addr, align 8
  store i32 %iCur, ptr %iCur.addr, align 4
  store ptr %pIdx, ptr %pIdx.addr, align 8
  call void @llvm.memset.p0.i64(ptr align 8 %w, i8 0, i64 48, i1 false)
  %0 = load i32, ptr %iCur.addr, align 4
  %iCur1 = getelementptr inbounds nuw %struct.IdxCover, ptr %xcov, i32 0, i32 1
  store i32 %0, ptr %iCur1, align 8
  %1 = load ptr, ptr %pIdx.addr, align 8
  %pIdx2 = getelementptr inbounds nuw %struct.IdxCover, ptr %xcov, i32 0, i32 0
  store ptr %1, ptr %pIdx2, align 8
  %xExprCallback = getelementptr inbounds nuw %struct.Walker, ptr %w, i32 0, i32 1
  store ptr @exprIdxCover, ptr %xExprCallback, align 8
  %u = getelementptr inbounds nuw %struct.Walker, ptr %w, i32 0, i32 6
  store ptr %xcov, ptr %u, align 8
  %2 = load ptr, ptr %pExpr.addr, align 8
  %call = call i32 @sqlite3WalkExpr(ptr noundef %w, ptr noundef %2)
  %eCode = getelementptr inbounds nuw %struct.Walker, ptr %w, i32 0, i32 5
  %3 = load i8, ptr %eCode, align 4
  %tobool = icmp ne i8 %3, 0
  %lnot = xor i1 %tobool, true
  %lnot.ext = zext i1 %lnot to i32
  ret i32 %lnot.ext
}

; Function Attrs: nounwind uwtable
declare hidden i32 @exprIdxCover(ptr noundef, ptr noundef) #1

attributes #0 = { nocallback nofree nounwind willreturn memory(argmem: write) }
attributes #1 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

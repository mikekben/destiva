; ModuleID = 'bench/sqlite3.ll'
source_filename = "sqlite-amalgamation-3300100/sqlite3.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

%struct.Window = type { ptr, ptr, ptr, ptr, i8, i8, i8, i8, i8, ptr, ptr, ptr, ptr, ptr, ptr, i32, i32, i32, i32, i32, i32, ptr, i32, i32, i32, i32, i32, i8 }

; Function Attrs: nounwind uwtable
declare hidden ptr @sqlite3WindowDup(ptr noundef, ptr noundef, ptr noundef) #0

; Function Attrs: nounwind uwtable
define hidden ptr @sqlite3WindowListDup(ptr noundef %db, ptr noundef %p) #0 {
entry:
  %db.addr = alloca ptr, align 8
  %p.addr = alloca ptr, align 8
  %pWin = alloca ptr, align 8
  %pRet = alloca ptr, align 8
  %pp = alloca ptr, align 8
  store ptr %db, ptr %db.addr, align 8
  store ptr %p, ptr %p.addr, align 8
  store ptr null, ptr %pRet, align 8
  store ptr %pRet, ptr %pp, align 8
  %0 = load ptr, ptr %p.addr, align 8
  store ptr %0, ptr %pWin, align 8
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %entry
  %1 = load ptr, ptr %pWin, align 8
  %tobool = icmp ne ptr %1, null
  br i1 %tobool, label %for.body, label %for.end

for.body:                                         ; preds = %for.cond
  %2 = load ptr, ptr %db.addr, align 8
  %3 = load ptr, ptr %pWin, align 8
  %call = call ptr @sqlite3WindowDup(ptr noundef %2, ptr noundef null, ptr noundef %3)
  %4 = load ptr, ptr %pp, align 8
  store ptr %call, ptr %4, align 8
  %5 = load ptr, ptr %pp, align 8
  %6 = load ptr, ptr %5, align 8
  %cmp = icmp eq ptr %6, null
  br i1 %cmp, label %if.then, label %if.end

if.then:                                          ; preds = %for.body
  br label %for.end

if.end:                                           ; preds = %for.body
  %7 = load ptr, ptr %pp, align 8
  %8 = load ptr, ptr %7, align 8
  %pNextWin = getelementptr inbounds nuw %struct.Window, ptr %8, i32 0, i32 12
  store ptr %pNextWin, ptr %pp, align 8
  br label %for.inc

for.inc:                                          ; preds = %if.end
  %9 = load ptr, ptr %pWin, align 8
  %pNextWin1 = getelementptr inbounds nuw %struct.Window, ptr %9, i32 0, i32 12
  %10 = load ptr, ptr %pNextWin1, align 8
  store ptr %10, ptr %pWin, align 8
  br label %for.cond, !llvm.loop !6

for.end:                                          ; preds = %if.then, %for.cond
  %11 = load ptr, ptr %pRet, align 8
  ret ptr %11
}

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}
!6 = distinct !{!6, !7}
!7 = !{!"llvm.loop.mustprogress"}

; ModuleID = 'bench/sqlite3.ll'
source_filename = "sqlite-amalgamation-3300100/sqlite3.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

%struct.Btree = type { ptr, ptr, i8, i8, i8, i8, i32, i32, i32, ptr, ptr, %struct.BtLock }
%struct.BtLock = type { ptr, i32, i8, ptr }
%struct.BtShared = type { ptr, ptr, ptr, ptr, i8, i8, i8, i8, i8, i8, i16, i16, i16, i16, i16, i32, i32, i32, i32, ptr, ptr, ptr, ptr, i32, ptr, ptr, ptr, ptr }
%struct.MemPage = type { i8, i8, i8, i8, i32, i8, i8, i8, i8, i8, i16, i16, i16, i32, i16, i16, [4 x i16], [4 x ptr], ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr }

; Function Attrs: nounwind uwtable
declare hidden void @sqlite3BtreeEnter(ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden void @sqlite3BtreeLeave(ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @sqlite3PagerDataVersion(ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @sqlite3Get4byte(ptr noundef) #0

; Function Attrs: nounwind uwtable
define hidden void @sqlite3BtreeGetMeta(ptr noundef %p, i32 noundef %idx, ptr noundef %pMeta) #0 {
entry:
  %p.addr = alloca ptr, align 8
  %idx.addr = alloca i32, align 4
  %pMeta.addr = alloca ptr, align 8
  %pBt = alloca ptr, align 8
  store ptr %p, ptr %p.addr, align 8
  store i32 %idx, ptr %idx.addr, align 4
  store ptr %pMeta, ptr %pMeta.addr, align 8
  %0 = load ptr, ptr %p.addr, align 8
  %pBt1 = getelementptr inbounds nuw %struct.Btree, ptr %0, i32 0, i32 1
  %1 = load ptr, ptr %pBt1, align 8
  store ptr %1, ptr %pBt, align 8
  %2 = load ptr, ptr %p.addr, align 8
  call void @sqlite3BtreeEnter(ptr noundef %2)
  %3 = load i32, ptr %idx.addr, align 4
  %cmp = icmp eq i32 %3, 15
  br i1 %cmp, label %if.then, label %if.else

if.then:                                          ; preds = %entry
  %4 = load ptr, ptr %pBt, align 8
  %pPager = getelementptr inbounds nuw %struct.BtShared, ptr %4, i32 0, i32 0
  %5 = load ptr, ptr %pPager, align 8
  %call = call i32 @sqlite3PagerDataVersion(ptr noundef %5)
  %6 = load ptr, ptr %p.addr, align 8
  %iDataVersion = getelementptr inbounds nuw %struct.Btree, ptr %6, i32 0, i32 8
  %7 = load i32, ptr %iDataVersion, align 4
  %add = add i32 %call, %7
  %8 = load ptr, ptr %pMeta.addr, align 8
  store i32 %add, ptr %8, align 4
  br label %if.end

if.else:                                          ; preds = %entry
  %9 = load ptr, ptr %pBt, align 8
  %pPage1 = getelementptr inbounds nuw %struct.BtShared, ptr %9, i32 0, i32 3
  %10 = load ptr, ptr %pPage1, align 8
  %aData = getelementptr inbounds nuw %struct.MemPage, ptr %10, i32 0, i32 19
  %11 = load ptr, ptr %aData, align 8
  %12 = load i32, ptr %idx.addr, align 4
  %mul = mul nsw i32 %12, 4
  %add2 = add nsw i32 36, %mul
  %idxprom = sext i32 %add2 to i64
  %arrayidx = getelementptr inbounds i8, ptr %11, i64 %idxprom
  %call3 = call i32 @sqlite3Get4byte(ptr noundef %arrayidx)
  %13 = load ptr, ptr %pMeta.addr, align 8
  store i32 %call3, ptr %13, align 4
  br label %if.end

if.end:                                           ; preds = %if.else, %if.then
  %14 = load ptr, ptr %p.addr, align 8
  call void @sqlite3BtreeLeave(ptr noundef %14)
  ret void
}

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

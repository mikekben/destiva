; ModuleID = 'bench/sqlite3.ll'
source_filename = "sqlite-amalgamation-3300100/sqlite3.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

%struct.Vdbe = type <{ ptr, ptr, ptr, ptr, i16, [2 x i8], i32, i32, i32, i32, i32, i32, i32, i32, [4 x i8], i64, i64, i64, i64, ptr, ptr, ptr, ptr, ptr, i32, i32, ptr, ptr, ptr, ptr, i64, i16, i8, i8, i8, i16, i8, i32, i32, [7 x i32], [4 x i8], ptr, ptr, ptr, ptr, i32, i32, ptr, ptr }>
%struct.sqlite3 = type { ptr, ptr, ptr, ptr, ptr, i32, i32, i64, i64, i64, i32, i32, i32, i32, i32, i16, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i32, i32, i32, i32, [12 x i32], i32, %struct.sqlite3InitInfo, i32, i32, i32, i32, i32, i32, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, ptr, %union.anon, %struct.Lookaside, ptr, ptr, ptr, ptr, i32, i32, %struct.Hash, ptr, ptr, ptr, %struct.Hash, %struct.Hash, %struct.BusyHandler, [2 x %struct.Db], ptr, i32, i32, i32, i64, i64, ptr }
%struct.sqlite3InitInfo = type { i32, i8, i8, i8, ptr }
%union.anon = type { double }
%struct.Lookaside = type { i32, i16, i8, i32, [3 x i32], ptr, ptr, ptr, ptr }
%struct.Hash = type { i32, i32, ptr, ptr }
%struct.BusyHandler = type { ptr, ptr, i32, i8 }
%struct.Db = type { ptr, ptr, i8, i8, ptr }

; Function Attrs: nounwind uwtable
declare dso_local void @sqlite3_mutex_enter(ptr noundef) #0

; Function Attrs: nounwind uwtable
declare dso_local void @sqlite3_mutex_leave(ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden void @invokeProfileCallback(ptr noundef, ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @sqlite3ApiExit(ptr noundef, i32 noundef) #0

; Function Attrs: nounwind uwtable
define dso_local i32 @sqlite3_reset(ptr noundef %pStmt) #0 {
entry:
  %pStmt.addr = alloca ptr, align 8
  %rc = alloca i32, align 4
  %v = alloca ptr, align 8
  %db = alloca ptr, align 8
  store ptr %pStmt, ptr %pStmt.addr, align 8
  %0 = load ptr, ptr %pStmt.addr, align 8
  %cmp = icmp eq ptr %0, null
  br i1 %cmp, label %if.then, label %if.else

if.then:                                          ; preds = %entry
  store i32 0, ptr %rc, align 4
  br label %if.end6

if.else:                                          ; preds = %entry
  %1 = load ptr, ptr %pStmt.addr, align 8
  store ptr %1, ptr %v, align 8
  %2 = load ptr, ptr %v, align 8
  %db1 = getelementptr inbounds nuw %struct.Vdbe, ptr %2, i32 0, i32 0
  %3 = load ptr, ptr %db1, align 8
  store ptr %3, ptr %db, align 8
  %4 = load ptr, ptr %db, align 8
  %mutex = getelementptr inbounds nuw %struct.sqlite3, ptr %4, i32 0, i32 3
  %5 = load ptr, ptr %mutex, align 8
  call void @sqlite3_mutex_enter(ptr noundef %5)
  %6 = load ptr, ptr %v, align 8
  %startTime = getelementptr inbounds nuw %struct.Vdbe, ptr %6, i32 0, i32 30
  %7 = load i64, ptr %startTime, align 8
  %cmp2 = icmp sgt i64 %7, 0
  br i1 %cmp2, label %if.then3, label %if.end

if.then3:                                         ; preds = %if.else
  %8 = load ptr, ptr %db, align 8
  %9 = load ptr, ptr %v, align 8
  call void @invokeProfileCallback(ptr noundef %8, ptr noundef %9)
  br label %if.end

if.end:                                           ; preds = %if.then3, %if.else
  %10 = load ptr, ptr %v, align 8
  %call = call i32 @sqlite3VdbeReset(ptr noundef %10)
  store i32 %call, ptr %rc, align 4
  %11 = load ptr, ptr %v, align 8
  call void @sqlite3VdbeRewind(ptr noundef %11)
  %12 = load ptr, ptr %db, align 8
  %13 = load i32, ptr %rc, align 4
  %call4 = call i32 @sqlite3ApiExit(ptr noundef %12, i32 noundef %13)
  store i32 %call4, ptr %rc, align 4
  %14 = load ptr, ptr %db, align 8
  %mutex5 = getelementptr inbounds nuw %struct.sqlite3, ptr %14, i32 0, i32 3
  %15 = load ptr, ptr %mutex5, align 8
  call void @sqlite3_mutex_leave(ptr noundef %15)
  br label %if.end6

if.end6:                                          ; preds = %if.end, %if.then
  %16 = load i32, ptr %rc, align 4
  ret i32 %16
}

; Function Attrs: nounwind uwtable
declare hidden i32 @sqlite3VdbeReset(ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden void @sqlite3VdbeRewind(ptr noundef) #0

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

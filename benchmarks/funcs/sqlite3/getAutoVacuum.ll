; ModuleID = 'bench/sqlite3.ll'
source_filename = "sqlite-amalgamation-3300100/sqlite3.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

@.str.860 = external hidden unnamed_addr constant [5 x i8], align 1
@.str.863 = external hidden unnamed_addr constant [5 x i8], align 1
@.str.864 = external hidden unnamed_addr constant [12 x i8], align 1

; Function Attrs: nounwind uwtable
declare hidden i32 @sqlite3StrICmp(ptr noundef, ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @sqlite3Atoi(ptr noundef) #0

; Function Attrs: nounwind uwtable
define hidden i32 @getAutoVacuum(ptr noundef %z) #0 {
entry:
  %retval = alloca i32, align 4
  %z.addr = alloca ptr, align 8
  %i = alloca i32, align 4
  store ptr %z, ptr %z.addr, align 8
  %0 = load ptr, ptr %z.addr, align 8
  %call = call i32 @sqlite3StrICmp(ptr noundef %0, ptr noundef @.str.863)
  %cmp = icmp eq i32 0, %call
  br i1 %cmp, label %if.then, label %if.end

if.then:                                          ; preds = %entry
  store i32 0, ptr %retval, align 4
  br label %return

if.end:                                           ; preds = %entry
  %1 = load ptr, ptr %z.addr, align 8
  %call1 = call i32 @sqlite3StrICmp(ptr noundef %1, ptr noundef @.str.860)
  %cmp2 = icmp eq i32 0, %call1
  br i1 %cmp2, label %if.then3, label %if.end4

if.then3:                                         ; preds = %if.end
  store i32 1, ptr %retval, align 4
  br label %return

if.end4:                                          ; preds = %if.end
  %2 = load ptr, ptr %z.addr, align 8
  %call5 = call i32 @sqlite3StrICmp(ptr noundef %2, ptr noundef @.str.864)
  %cmp6 = icmp eq i32 0, %call5
  br i1 %cmp6, label %if.then7, label %if.end8

if.then7:                                         ; preds = %if.end4
  store i32 2, ptr %retval, align 4
  br label %return

if.end8:                                          ; preds = %if.end4
  %3 = load ptr, ptr %z.addr, align 8
  %call9 = call i32 @sqlite3Atoi(ptr noundef %3)
  store i32 %call9, ptr %i, align 4
  %4 = load i32, ptr %i, align 4
  %cmp10 = icmp sge i32 %4, 0
  br i1 %cmp10, label %land.lhs.true, label %cond.false

land.lhs.true:                                    ; preds = %if.end8
  %5 = load i32, ptr %i, align 4
  %cmp11 = icmp sle i32 %5, 2
  br i1 %cmp11, label %cond.true, label %cond.false

cond.true:                                        ; preds = %land.lhs.true
  %6 = load i32, ptr %i, align 4
  br label %cond.end

cond.false:                                       ; preds = %land.lhs.true, %if.end8
  br label %cond.end

cond.end:                                         ; preds = %cond.false, %cond.true
  %cond = phi i32 [ %6, %cond.true ], [ 0, %cond.false ]
  %conv = trunc i32 %cond to i8
  %conv12 = zext i8 %conv to i32
  store i32 %conv12, ptr %retval, align 4
  br label %return

return:                                           ; preds = %cond.end, %if.then7, %if.then3, %if.then
  %7 = load i32, ptr %retval, align 4
  ret i32 %7
}

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

; ModuleID = 'bench/sqlite3.ll'
source_filename = "sqlite-amalgamation-3300100/sqlite3.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

%struct.Pager = type { ptr, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i8, i32, i32, i32, i32, i32, i32, i32, i32, ptr, ptr, ptr, ptr, i64, i64, ptr, ptr, i32, i32, [16 x i8], i32, i64, ptr, i16, i16, i32, i32, i32, i32, i64, ptr, ptr, ptr, ptr, [4 x i32], ptr, ptr, ptr, ptr, ptr, ptr }

; Function Attrs: nounwind uwtable
declare hidden void @sqlite3WalEndReadTransaction(ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @sqlite3OsUnfetch(ptr noundef, i64 noundef, ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden void @pager_reset(ptr noundef) #0

; Function Attrs: nounwind uwtable
define hidden i32 @pagerBeginReadTransaction(ptr noundef %pPager) #0 {
entry:
  %pPager.addr = alloca ptr, align 8
  %rc = alloca i32, align 4
  %changed = alloca i32, align 4
  store ptr %pPager, ptr %pPager.addr, align 8
  store i32 0, ptr %changed, align 4
  %0 = load ptr, ptr %pPager.addr, align 8
  %pWal = getelementptr inbounds nuw %struct.Pager, ptr %0, i32 0, i32 59
  %1 = load ptr, ptr %pWal, align 8
  call void @sqlite3WalEndReadTransaction(ptr noundef %1)
  %2 = load ptr, ptr %pPager.addr, align 8
  %pWal1 = getelementptr inbounds nuw %struct.Pager, ptr %2, i32 0, i32 59
  %3 = load ptr, ptr %pWal1, align 8
  %call = call i32 @sqlite3WalBeginReadTransaction(ptr noundef %3, ptr noundef %changed)
  store i32 %call, ptr %rc, align 4
  %4 = load i32, ptr %rc, align 4
  %cmp = icmp ne i32 %4, 0
  br i1 %cmp, label %if.then, label %lor.lhs.false

lor.lhs.false:                                    ; preds = %entry
  %5 = load i32, ptr %changed, align 4
  %tobool = icmp ne i32 %5, 0
  br i1 %tobool, label %if.then, label %if.end5

if.then:                                          ; preds = %lor.lhs.false, %entry
  %6 = load ptr, ptr %pPager.addr, align 8
  call void @pager_reset(ptr noundef %6)
  %7 = load ptr, ptr %pPager.addr, align 8
  %bUseFetch = getelementptr inbounds nuw %struct.Pager, ptr %7, i32 0, i32 19
  %8 = load i8, ptr %bUseFetch, align 2
  %tobool2 = icmp ne i8 %8, 0
  br i1 %tobool2, label %if.then3, label %if.end

if.then3:                                         ; preds = %if.then
  %9 = load ptr, ptr %pPager.addr, align 8
  %fd = getelementptr inbounds nuw %struct.Pager, ptr %9, i32 0, i32 30
  %10 = load ptr, ptr %fd, align 8
  %call4 = call i32 @sqlite3OsUnfetch(ptr noundef %10, i64 noundef 0, ptr noundef null)
  br label %if.end

if.end:                                           ; preds = %if.then3, %if.then
  br label %if.end5

if.end5:                                          ; preds = %if.end, %lor.lhs.false
  %11 = load i32, ptr %rc, align 4
  ret i32 %11
}

; Function Attrs: nounwind uwtable
declare hidden i32 @sqlite3WalBeginReadTransaction(ptr noundef, ptr noundef) #0

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

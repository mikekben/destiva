; ModuleID = 'bench/ph7.ll'
source_filename = "ph7/ph7.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

; Function Attrs: nounwind uwtable
define hidden i32 @SyHashDeleteEntry(ptr noundef %pHash, ptr noundef %pKey, i32 noundef %nKeyLen, ptr noundef %ppUserData) #0 {
entry:
  %retval = alloca i32, align 4
  %pHash.addr = alloca ptr, align 8
  %pKey.addr = alloca ptr, align 8
  %nKeyLen.addr = alloca i32, align 4
  %ppUserData.addr = alloca ptr, align 8
  %pEntry = alloca ptr, align 8
  %rc = alloca i32, align 4
  store ptr %pHash, ptr %pHash.addr, align 8
  store ptr %pKey, ptr %pKey.addr, align 8
  store i32 %nKeyLen, ptr %nKeyLen.addr, align 4
  store ptr %ppUserData, ptr %ppUserData.addr, align 8
  %0 = load ptr, ptr %pHash.addr, align 8
  %1 = load ptr, ptr %pKey.addr, align 8
  %2 = load i32, ptr %nKeyLen.addr, align 4
  %call = call ptr @HashGetEntry(ptr noundef %0, ptr noundef %1, i32 noundef %2)
  store ptr %call, ptr %pEntry, align 8
  %3 = load ptr, ptr %pEntry, align 8
  %cmp = icmp eq ptr %3, null
  br i1 %cmp, label %if.then, label %if.end

if.then:                                          ; preds = %entry
  store i32 -6, ptr %retval, align 4
  br label %return

if.end:                                           ; preds = %entry
  %4 = load ptr, ptr %pHash.addr, align 8
  %5 = load ptr, ptr %pEntry, align 8
  %6 = load ptr, ptr %ppUserData.addr, align 8
  %call1 = call i32 @HashDeleteEntry(ptr noundef %4, ptr noundef %5, ptr noundef %6)
  store i32 %call1, ptr %rc, align 4
  %7 = load i32, ptr %rc, align 4
  store i32 %7, ptr %retval, align 4
  br label %return

return:                                           ; preds = %if.end, %if.then
  %8 = load i32, ptr %retval, align 4
  ret i32 %8
}

; Function Attrs: nounwind uwtable
declare hidden ptr @HashGetEntry(ptr noundef, ptr noundef, i32 noundef) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @HashDeleteEntry(ptr noundef, ptr noundef, ptr noundef) #0

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

; ModuleID = 'bench/ph7.ll'
source_filename = "ph7/ph7.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

; Function Attrs: nounwind uwtable
declare hidden ptr @SyMemBackendAlloc(ptr noundef, i32 noundef) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @SyMemcpy(ptr noundef, ptr noundef, i32 noundef) #0

; Function Attrs: nounwind uwtable
define hidden ptr @SyMemBackendDup(ptr noundef %pBackend, ptr noundef %pSrc, i32 noundef %nSize) #0 {
entry:
  %pBackend.addr = alloca ptr, align 8
  %pSrc.addr = alloca ptr, align 8
  %nSize.addr = alloca i32, align 4
  %pNew = alloca ptr, align 8
  store ptr %pBackend, ptr %pBackend.addr, align 8
  store ptr %pSrc, ptr %pSrc.addr, align 8
  store i32 %nSize, ptr %nSize.addr, align 4
  %0 = load ptr, ptr %pBackend.addr, align 8
  %1 = load i32, ptr %nSize.addr, align 4
  %call = call ptr @SyMemBackendAlloc(ptr noundef %0, i32 noundef %1)
  store ptr %call, ptr %pNew, align 8
  %2 = load ptr, ptr %pNew, align 8
  %tobool = icmp ne ptr %2, null
  br i1 %tobool, label %if.then, label %if.end

if.then:                                          ; preds = %entry
  %3 = load ptr, ptr %pSrc.addr, align 8
  %4 = load ptr, ptr %pNew, align 8
  %5 = load i32, ptr %nSize.addr, align 4
  %call1 = call i32 @SyMemcpy(ptr noundef %3, ptr noundef %4, i32 noundef %5)
  br label %if.end

if.end:                                           ; preds = %if.then, %entry
  %6 = load ptr, ptr %pNew, align 8
  ret ptr %6
}

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

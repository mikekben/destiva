; ModuleID = 'bench/ph7.ll'
source_filename = "ph7/ph7.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

; Function Attrs: nounwind uwtable
define hidden i32 @UnixFile_Lock(ptr noundef %pUserData, i32 noundef %lock_type) #0 {
entry:
  %pUserData.addr = alloca ptr, align 8
  %lock_type.addr = alloca i32, align 4
  %fd = alloca i32, align 4
  %rc = alloca i32, align 4
  store ptr %pUserData, ptr %pUserData.addr, align 8
  store i32 %lock_type, ptr %lock_type.addr, align 4
  %0 = load ptr, ptr %pUserData.addr, align 8
  %1 = ptrtoint ptr %0 to i64
  %conv = trunc i64 %1 to i32
  store i32 %conv, ptr %fd, align 4
  store i32 0, ptr %rc, align 4
  %2 = load i32, ptr %lock_type.addr, align 4
  %cmp = icmp slt i32 %2, 0
  br i1 %cmp, label %if.then, label %if.else

if.then:                                          ; preds = %entry
  %3 = load i32, ptr %fd, align 4
  %call = call i32 @flock(i32 noundef %3, i32 noundef 8) #2
  store i32 %call, ptr %rc, align 4
  br label %if.end8

if.else:                                          ; preds = %entry
  %4 = load i32, ptr %lock_type.addr, align 4
  %cmp2 = icmp eq i32 %4, 1
  br i1 %cmp2, label %if.then4, label %if.else6

if.then4:                                         ; preds = %if.else
  %5 = load i32, ptr %fd, align 4
  %call5 = call i32 @flock(i32 noundef %5, i32 noundef 2) #2
  store i32 %call5, ptr %rc, align 4
  br label %if.end

if.else6:                                         ; preds = %if.else
  %6 = load i32, ptr %fd, align 4
  %call7 = call i32 @flock(i32 noundef %6, i32 noundef 1) #2
  store i32 %call7, ptr %rc, align 4
  br label %if.end

if.end:                                           ; preds = %if.else6, %if.then4
  br label %if.end8

if.end8:                                          ; preds = %if.end, %if.then
  %7 = load i32, ptr %rc, align 4
  %tobool = icmp ne i32 %7, 0
  %lnot = xor i1 %tobool, true
  %8 = zext i1 %lnot to i64
  %cond = select i1 %lnot, i32 0, i32 -1
  ret i32 %cond
}

; Function Attrs: nounwind
declare i32 @flock(i32 noundef, i32 noundef) #1

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #1 = { nounwind "frame-pointer"="all" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #2 = { nounwind }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

; ModuleID = 'bench/ph7.ll'
source_filename = "ph7/ph7.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

%struct.ph7_value = type { double, %union.anon, i32, ptr, %struct.SyBlob, i32 }
%union.anon = type { i64 }
%struct.SyBlob = type { ptr, ptr, i32, i32, i32 }
%struct.ph7_hashmap = type { ptr, ptr, ptr, ptr, ptr, i32, i32, ptr, ptr, i64, i32 }

; Function Attrs: nounwind uwtable
declare hidden i32 @PH7_MemObjRelease(ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @PH7_MemObjStringAppend(ptr noundef, ptr noundef, i32 noundef) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @PH7_MemObjInitFromString(ptr noundef, ptr noundef, ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @PH7_HashmapInsertByRef(ptr noundef, ptr noundef, i32 noundef) #0

; Function Attrs: nounwind uwtable
define hidden i32 @VmHashmapRefInsert(ptr noundef %pMap, ptr noundef %zKey, i32 noundef %nByte, i32 noundef %nRefIdx) #0 {
entry:
  %pMap.addr = alloca ptr, align 8
  %zKey.addr = alloca ptr, align 8
  %nByte.addr = alloca i32, align 4
  %nRefIdx.addr = alloca i32, align 4
  %sKey = alloca %struct.ph7_value, align 8
  %rc = alloca i32, align 4
  store ptr %pMap, ptr %pMap.addr, align 8
  store ptr %zKey, ptr %zKey.addr, align 8
  store i32 %nByte, ptr %nByte.addr, align 4
  store i32 %nRefIdx, ptr %nRefIdx.addr, align 4
  %0 = load ptr, ptr %pMap.addr, align 8
  %pVm = getelementptr inbounds nuw %struct.ph7_hashmap, ptr %0, i32 0, i32 0
  %1 = load ptr, ptr %pVm, align 8
  %call = call i32 @PH7_MemObjInitFromString(ptr noundef %1, ptr noundef %sKey, ptr noundef null)
  %2 = load ptr, ptr %zKey.addr, align 8
  %3 = load i32, ptr %nByte.addr, align 4
  %call1 = call i32 @PH7_MemObjStringAppend(ptr noundef %sKey, ptr noundef %2, i32 noundef %3)
  %4 = load ptr, ptr %pMap.addr, align 8
  %5 = load i32, ptr %nRefIdx.addr, align 4
  %call2 = call i32 @PH7_HashmapInsertByRef(ptr noundef %4, ptr noundef %sKey, i32 noundef %5)
  store i32 %call2, ptr %rc, align 4
  %call3 = call i32 @PH7_MemObjRelease(ptr noundef %sKey)
  %6 = load i32, ptr %rc, align 4
  ret i32 %6
}

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

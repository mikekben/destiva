; ModuleID = 'bench/ph7.ll'
source_filename = "ph7/ph7.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

%struct.ph7_class = type { ptr, %struct.SyHash, %struct.SyString, i32, %struct.SyHash, %struct.SyHash, i32, %struct.SySet, ptr }
%struct.SyString = type { ptr, i32 }
%struct.SyHash = type { ptr, ptr, ptr, ptr, ptr, i32, ptr, i32 }
%struct.SySet = type { ptr, ptr, i32, i32, i32, i32, ptr }

; Function Attrs: nounwind uwtable
declare dso_local i32 @ph7_value_string(ptr noundef, ptr noundef, i32 noundef) #0

; Function Attrs: nounwind uwtable
declare dso_local i32 @ph7_value_null(ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden ptr @PH7_VmPeekTopClass(ptr noundef) #0

; Function Attrs: nounwind uwtable
define hidden void @PH7_parent_Const(ptr noundef %pVal, ptr noundef %pUserData) #0 {
entry:
  %pVal.addr = alloca ptr, align 8
  %pUserData.addr = alloca ptr, align 8
  %pVm = alloca ptr, align 8
  %pClass = alloca ptr, align 8
  %pName = alloca ptr, align 8
  store ptr %pVal, ptr %pVal.addr, align 8
  store ptr %pUserData, ptr %pUserData.addr, align 8
  %0 = load ptr, ptr %pUserData.addr, align 8
  store ptr %0, ptr %pVm, align 8
  %1 = load ptr, ptr %pVm, align 8
  %call = call ptr @PH7_VmPeekTopClass(ptr noundef %1)
  store ptr %call, ptr %pClass, align 8
  %2 = load ptr, ptr %pClass, align 8
  %tobool = icmp ne ptr %2, null
  br i1 %tobool, label %land.lhs.true, label %if.else

land.lhs.true:                                    ; preds = %entry
  %3 = load ptr, ptr %pClass, align 8
  %pBase = getelementptr inbounds nuw %struct.ph7_class, ptr %3, i32 0, i32 0
  %4 = load ptr, ptr %pBase, align 8
  %tobool1 = icmp ne ptr %4, null
  br i1 %tobool1, label %if.then, label %if.else

if.then:                                          ; preds = %land.lhs.true
  %5 = load ptr, ptr %pClass, align 8
  %pBase2 = getelementptr inbounds nuw %struct.ph7_class, ptr %5, i32 0, i32 0
  %6 = load ptr, ptr %pBase2, align 8
  %sName = getelementptr inbounds nuw %struct.ph7_class, ptr %6, i32 0, i32 2
  store ptr %sName, ptr %pName, align 8
  %7 = load ptr, ptr %pVal.addr, align 8
  %8 = load ptr, ptr %pName, align 8
  %zString = getelementptr inbounds nuw %struct.SyString, ptr %8, i32 0, i32 0
  %9 = load ptr, ptr %zString, align 8
  %10 = load ptr, ptr %pName, align 8
  %nByte = getelementptr inbounds nuw %struct.SyString, ptr %10, i32 0, i32 1
  %11 = load i32, ptr %nByte, align 8
  %call3 = call i32 @ph7_value_string(ptr noundef %7, ptr noundef %9, i32 noundef %11)
  br label %if.end

if.else:                                          ; preds = %land.lhs.true, %entry
  %12 = load ptr, ptr %pVal.addr, align 8
  %call4 = call i32 @ph7_value_null(ptr noundef %12)
  br label %if.end

if.end:                                           ; preds = %if.else, %if.then
  ret void
}

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

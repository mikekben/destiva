; ModuleID = 'bench/ph7.ll'
source_filename = "ph7/ph7.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

%struct.SyMemBackend = type { ptr, ptr, ptr, i32, ptr, ptr, ptr, i32, [15 x ptr] }
%struct.SyMutexMethods = type { ptr, ptr, ptr, ptr, ptr, ptr, ptr }

; Function Attrs: nounwind uwtable
define hidden i32 @SyMemBackendRelease(ptr noundef %pBackend) #0 {
entry:
  %pBackend.addr = alloca ptr, align 8
  %rc = alloca i32, align 4
  store ptr %pBackend, ptr %pBackend.addr, align 8
  %0 = load ptr, ptr %pBackend.addr, align 8
  %pMutexMethods = getelementptr inbounds nuw %struct.SyMemBackend, ptr %0, i32 0, i32 0
  %1 = load ptr, ptr %pMutexMethods, align 8
  %tobool = icmp ne ptr %1, null
  br i1 %tobool, label %if.then, label %if.end5

if.then:                                          ; preds = %entry
  %2 = load ptr, ptr %pBackend.addr, align 8
  %pMutex = getelementptr inbounds nuw %struct.SyMemBackend, ptr %2, i32 0, i32 6
  %3 = load ptr, ptr %pMutex, align 8
  %tobool1 = icmp ne ptr %3, null
  br i1 %tobool1, label %if.then2, label %if.end

if.then2:                                         ; preds = %if.then
  %4 = load ptr, ptr %pBackend.addr, align 8
  %pMutexMethods3 = getelementptr inbounds nuw %struct.SyMemBackend, ptr %4, i32 0, i32 0
  %5 = load ptr, ptr %pMutexMethods3, align 8
  %xEnter = getelementptr inbounds nuw %struct.SyMutexMethods, ptr %5, i32 0, i32 4
  %6 = load ptr, ptr %xEnter, align 8
  %7 = load ptr, ptr %pBackend.addr, align 8
  %pMutex4 = getelementptr inbounds nuw %struct.SyMemBackend, ptr %7, i32 0, i32 6
  %8 = load ptr, ptr %pMutex4, align 8
  call void %6(ptr noundef %8)
  br label %if.end

if.end:                                           ; preds = %if.then2, %if.then
  br label %if.end5

if.end5:                                          ; preds = %if.end, %entry
  %9 = load ptr, ptr %pBackend.addr, align 8
  %call = call i32 @MemBackendRelease(ptr noundef %9)
  store i32 %call, ptr %rc, align 4
  %10 = load ptr, ptr %pBackend.addr, align 8
  %pMutexMethods6 = getelementptr inbounds nuw %struct.SyMemBackend, ptr %10, i32 0, i32 0
  %11 = load ptr, ptr %pMutexMethods6, align 8
  %tobool7 = icmp ne ptr %11, null
  br i1 %tobool7, label %if.then8, label %if.end24

if.then8:                                         ; preds = %if.end5
  %12 = load ptr, ptr %pBackend.addr, align 8
  %pMutex9 = getelementptr inbounds nuw %struct.SyMemBackend, ptr %12, i32 0, i32 6
  %13 = load ptr, ptr %pMutex9, align 8
  %tobool10 = icmp ne ptr %13, null
  br i1 %tobool10, label %if.then11, label %if.end14

if.then11:                                        ; preds = %if.then8
  %14 = load ptr, ptr %pBackend.addr, align 8
  %pMutexMethods12 = getelementptr inbounds nuw %struct.SyMemBackend, ptr %14, i32 0, i32 0
  %15 = load ptr, ptr %pMutexMethods12, align 8
  %xLeave = getelementptr inbounds nuw %struct.SyMutexMethods, ptr %15, i32 0, i32 6
  %16 = load ptr, ptr %xLeave, align 8
  %17 = load ptr, ptr %pBackend.addr, align 8
  %pMutex13 = getelementptr inbounds nuw %struct.SyMemBackend, ptr %17, i32 0, i32 6
  %18 = load ptr, ptr %pMutex13, align 8
  call void %16(ptr noundef %18)
  br label %if.end14

if.end14:                                         ; preds = %if.then11, %if.then8
  %19 = load ptr, ptr %pBackend.addr, align 8
  %pMutex15 = getelementptr inbounds nuw %struct.SyMemBackend, ptr %19, i32 0, i32 6
  %20 = load ptr, ptr %pMutex15, align 8
  %tobool16 = icmp ne ptr %20, null
  br i1 %tobool16, label %land.lhs.true, label %if.end23

land.lhs.true:                                    ; preds = %if.end14
  %21 = load ptr, ptr %pBackend.addr, align 8
  %pMutexMethods17 = getelementptr inbounds nuw %struct.SyMemBackend, ptr %21, i32 0, i32 0
  %22 = load ptr, ptr %pMutexMethods17, align 8
  %xRelease = getelementptr inbounds nuw %struct.SyMutexMethods, ptr %22, i32 0, i32 3
  %23 = load ptr, ptr %xRelease, align 8
  %tobool18 = icmp ne ptr %23, null
  br i1 %tobool18, label %if.then19, label %if.end23

if.then19:                                        ; preds = %land.lhs.true
  %24 = load ptr, ptr %pBackend.addr, align 8
  %pMutexMethods20 = getelementptr inbounds nuw %struct.SyMemBackend, ptr %24, i32 0, i32 0
  %25 = load ptr, ptr %pMutexMethods20, align 8
  %xRelease21 = getelementptr inbounds nuw %struct.SyMutexMethods, ptr %25, i32 0, i32 3
  %26 = load ptr, ptr %xRelease21, align 8
  %27 = load ptr, ptr %pBackend.addr, align 8
  %pMutex22 = getelementptr inbounds nuw %struct.SyMemBackend, ptr %27, i32 0, i32 6
  %28 = load ptr, ptr %pMutex22, align 8
  call void %26(ptr noundef %28)
  br label %if.end23

if.end23:                                         ; preds = %if.then19, %land.lhs.true, %if.end14
  br label %if.end24

if.end24:                                         ; preds = %if.end23, %if.end5
  ret i32 0
}

; Function Attrs: nounwind uwtable
declare hidden i32 @MemBackendRelease(ptr noundef) #0

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

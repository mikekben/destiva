; ModuleID = 'bench/ph7.ll'
source_filename = "ph7/ph7.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

%struct.ph7_hashmap = type { ptr, ptr, ptr, ptr, ptr, i32, i32, ptr, ptr, i64, i32 }
%struct.ph7_hashmap_node = type { ptr, i32, %union.anon.0, i32, i32, i32, ptr, ptr, ptr, ptr }
%union.anon.0 = type { %struct.SyBlob }
%struct.SyBlob = type { ptr, ptr, i32, i32, i32 }

; Function Attrs: nounwind uwtable
declare dso_local i32 @ph7_result_bool(ptr noundef, i32 noundef) #0

; Function Attrs: nounwind uwtable
declare dso_local i32 @ph7_result_value(ptr noundef, ptr noundef) #0

; Function Attrs: nounwind uwtable
declare hidden ptr @HashmapExtractNodeValue(ptr noundef) #0

; Function Attrs: nounwind uwtable
define hidden i32 @HashmapCurrentValue(ptr noundef %pCtx, ptr noundef %pMap, i32 noundef %iDirection) #0 {
entry:
  %retval = alloca i32, align 4
  %pCtx.addr = alloca ptr, align 8
  %pMap.addr = alloca ptr, align 8
  %iDirection.addr = alloca i32, align 4
  %pCur = alloca ptr, align 8
  %pVal = alloca ptr, align 8
  store ptr %pCtx, ptr %pCtx.addr, align 8
  store ptr %pMap, ptr %pMap.addr, align 8
  store i32 %iDirection, ptr %iDirection.addr, align 4
  %0 = load ptr, ptr %pMap.addr, align 8
  %pCur1 = getelementptr inbounds nuw %struct.ph7_hashmap, ptr %0, i32 0, i32 4
  %1 = load ptr, ptr %pCur1, align 8
  store ptr %1, ptr %pCur, align 8
  %2 = load ptr, ptr %pCur, align 8
  %cmp = icmp eq ptr %2, null
  br i1 %cmp, label %if.then, label %if.end

if.then:                                          ; preds = %entry
  %3 = load ptr, ptr %pCtx.addr, align 8
  %call = call i32 @ph7_result_bool(ptr noundef %3, i32 noundef 0)
  store i32 0, ptr %retval, align 4
  br label %return

if.end:                                           ; preds = %entry
  %4 = load i32, ptr %iDirection.addr, align 4
  %cmp2 = icmp ne i32 %4, 0
  br i1 %cmp2, label %if.then3, label %if.end15

if.then3:                                         ; preds = %if.end
  %5 = load i32, ptr %iDirection.addr, align 4
  %cmp4 = icmp sgt i32 %5, 0
  br i1 %cmp4, label %if.then5, label %if.else

if.then5:                                         ; preds = %if.then3
  %6 = load ptr, ptr %pCur, align 8
  %pPrev = getelementptr inbounds nuw %struct.ph7_hashmap_node, ptr %6, i32 0, i32 7
  %7 = load ptr, ptr %pPrev, align 8
  %8 = load ptr, ptr %pMap.addr, align 8
  %pCur6 = getelementptr inbounds nuw %struct.ph7_hashmap, ptr %8, i32 0, i32 4
  store ptr %7, ptr %pCur6, align 8
  %9 = load ptr, ptr %pMap.addr, align 8
  %pCur7 = getelementptr inbounds nuw %struct.ph7_hashmap, ptr %9, i32 0, i32 4
  %10 = load ptr, ptr %pCur7, align 8
  store ptr %10, ptr %pCur, align 8
  br label %if.end10

if.else:                                          ; preds = %if.then3
  %11 = load ptr, ptr %pCur, align 8
  %pNext = getelementptr inbounds nuw %struct.ph7_hashmap_node, ptr %11, i32 0, i32 6
  %12 = load ptr, ptr %pNext, align 8
  %13 = load ptr, ptr %pMap.addr, align 8
  %pCur8 = getelementptr inbounds nuw %struct.ph7_hashmap, ptr %13, i32 0, i32 4
  store ptr %12, ptr %pCur8, align 8
  %14 = load ptr, ptr %pMap.addr, align 8
  %pCur9 = getelementptr inbounds nuw %struct.ph7_hashmap, ptr %14, i32 0, i32 4
  %15 = load ptr, ptr %pCur9, align 8
  store ptr %15, ptr %pCur, align 8
  br label %if.end10

if.end10:                                         ; preds = %if.else, %if.then5
  %16 = load ptr, ptr %pCur, align 8
  %cmp11 = icmp eq ptr %16, null
  br i1 %cmp11, label %if.then12, label %if.end14

if.then12:                                        ; preds = %if.end10
  %17 = load ptr, ptr %pCtx.addr, align 8
  %call13 = call i32 @ph7_result_bool(ptr noundef %17, i32 noundef 0)
  store i32 0, ptr %retval, align 4
  br label %return

if.end14:                                         ; preds = %if.end10
  br label %if.end15

if.end15:                                         ; preds = %if.end14, %if.end
  %18 = load ptr, ptr %pCur, align 8
  %call16 = call ptr @HashmapExtractNodeValue(ptr noundef %18)
  store ptr %call16, ptr %pVal, align 8
  %19 = load ptr, ptr %pVal, align 8
  %tobool = icmp ne ptr %19, null
  br i1 %tobool, label %if.then17, label %if.else19

if.then17:                                        ; preds = %if.end15
  %20 = load ptr, ptr %pCtx.addr, align 8
  %21 = load ptr, ptr %pVal, align 8
  %call18 = call i32 @ph7_result_value(ptr noundef %20, ptr noundef %21)
  br label %if.end21

if.else19:                                        ; preds = %if.end15
  %22 = load ptr, ptr %pCtx.addr, align 8
  %call20 = call i32 @ph7_result_bool(ptr noundef %22, i32 noundef 0)
  br label %if.end21

if.end21:                                         ; preds = %if.else19, %if.then17
  store i32 0, ptr %retval, align 4
  br label %return

return:                                           ; preds = %if.end21, %if.then12, %if.then
  %23 = load i32, ptr %retval, align 4
  ret i32 %23
}

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

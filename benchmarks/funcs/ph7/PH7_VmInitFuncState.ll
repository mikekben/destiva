; ModuleID = 'bench/ph7.ll'
source_filename = "ph7/ph7.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-unknown-linux-gnu"

%struct.ph7_vm_func = type { %struct.SySet, %struct.SySet, %struct.SyString, %struct.SySet, %struct.SySet, i32, %struct.SyString, ptr, ptr }
%struct.SySet = type { ptr, ptr, i32, i32, i32, i32, ptr }
%struct.SyString = type { ptr, i32 }
%struct.ph7_vm = type { %struct.SyMemBackend, ptr, %struct.SySet, ptr, ptr, %struct.SyPRNGCtx, %struct.SySet, %struct.SySet, ptr, %struct.SySet, %struct.SyHash, %struct.SyHash, %struct.SyHash, %struct.SyHash, %struct.SyHash, %struct.SyHash, %struct.SyBlob, %struct.SyBlob, %struct.SyBlob, %struct.SySet, %struct.SySet, %struct.SySet, %struct.SySet, %struct.SySet, %struct.SySet, %struct.SySet, ptr, %struct.ph7_value, [2 x %struct.ph7_value], [2 x %struct.ph7_value], ptr, ptr, ptr, i32, i32, i32, i32, i32, i32, i32, i32, ptr, i32, %struct.ph7_output_consumer, i32, %struct.ph7_value, ptr, ptr, i32, i32, %struct.SySet, ptr, i32, i32, %struct.ph7_gen_state, ptr, ptr, i32 }
%struct.SyMemBackend = type { ptr, ptr, ptr, i32, ptr, ptr, ptr, i32, [15 x ptr] }
%struct.SyPRNGCtx = type { i8, i8, [256 x i8], i16 }
%struct.SyHash = type { ptr, ptr, ptr, ptr, ptr, i32, ptr, i32 }
%struct.SyBlob = type { ptr, ptr, i32, i32, i32 }
%struct.ph7_output_consumer = type { ptr, ptr, ptr, ptr }
%struct.ph7_value = type { double, %union.anon, i32, ptr, %struct.SyBlob, i32 }
%union.anon = type { i64 }
%struct.ph7_gen_state = type { ptr, %struct.SyHash, %struct.SyHash, %struct.SyHash, ptr, %struct.GenBlock, ptr, ptr, %struct.SySet, %struct.SySet, %struct.SyBlob, %struct.SyBlob, ptr, ptr, i32, ptr, ptr, ptr }
%struct.GenBlock = type { ptr, ptr, i32, i32, %struct.SySet, ptr, i8, %struct.SySet }

; Function Attrs: nounwind uwtable
declare hidden void @SyZero(ptr noundef, i32 noundef) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @SySetInit(ptr noundef, ptr noundef, i32 noundef) #0

; Function Attrs: nounwind uwtable
declare hidden i32 @SySetAlloc(ptr noundef, i32 noundef) #0

; Function Attrs: nounwind uwtable
define hidden i32 @PH7_VmInitFuncState(ptr noundef %pVm, ptr noundef %pFunc, ptr noundef %zName, i32 noundef %nByte, i32 noundef %iFlags, ptr noundef %pUserData) #0 {
entry:
  %pVm.addr = alloca ptr, align 8
  %pFunc.addr = alloca ptr, align 8
  %zName.addr = alloca ptr, align 8
  %nByte.addr = alloca i32, align 4
  %iFlags.addr = alloca i32, align 4
  %pUserData.addr = alloca ptr, align 8
  store ptr %pVm, ptr %pVm.addr, align 8
  store ptr %pFunc, ptr %pFunc.addr, align 8
  store ptr %zName, ptr %zName.addr, align 8
  store i32 %nByte, ptr %nByte.addr, align 4
  store i32 %iFlags, ptr %iFlags.addr, align 4
  store ptr %pUserData, ptr %pUserData.addr, align 8
  %0 = load ptr, ptr %pFunc.addr, align 8
  call void @SyZero(ptr noundef %0, i32 noundef 216)
  %1 = load ptr, ptr %pFunc.addr, align 8
  %aArgs = getelementptr inbounds nuw %struct.ph7_vm_func, ptr %1, i32 0, i32 0
  %2 = load ptr, ptr %pVm.addr, align 8
  %sAllocator = getelementptr inbounds nuw %struct.ph7_vm, ptr %2, i32 0, i32 0
  %call = call i32 @SySetInit(ptr noundef %aArgs, ptr noundef %sAllocator, i32 noundef 88)
  %3 = load ptr, ptr %pFunc.addr, align 8
  %aStatic = getelementptr inbounds nuw %struct.ph7_vm_func, ptr %3, i32 0, i32 1
  %4 = load ptr, ptr %pVm.addr, align 8
  %sAllocator1 = getelementptr inbounds nuw %struct.ph7_vm, ptr %4, i32 0, i32 0
  %call2 = call i32 @SySetInit(ptr noundef %aStatic, ptr noundef %sAllocator1, i32 noundef 64)
  %5 = load ptr, ptr %pFunc.addr, align 8
  %aByteCode = getelementptr inbounds nuw %struct.ph7_vm_func, ptr %5, i32 0, i32 3
  %6 = load ptr, ptr %pVm.addr, align 8
  %sAllocator3 = getelementptr inbounds nuw %struct.ph7_vm, ptr %6, i32 0, i32 0
  %call4 = call i32 @SySetInit(ptr noundef %aByteCode, ptr noundef %sAllocator3, i32 noundef 24)
  %7 = load ptr, ptr %pFunc.addr, align 8
  %aByteCode5 = getelementptr inbounds nuw %struct.ph7_vm_func, ptr %7, i32 0, i32 3
  %call6 = call i32 @SySetAlloc(ptr noundef %aByteCode5, i32 noundef 16)
  %8 = load ptr, ptr %pFunc.addr, align 8
  %aClosureEnv = getelementptr inbounds nuw %struct.ph7_vm_func, ptr %8, i32 0, i32 4
  %9 = load ptr, ptr %pVm.addr, align 8
  %sAllocator7 = getelementptr inbounds nuw %struct.ph7_vm, ptr %9, i32 0, i32 0
  %call8 = call i32 @SySetInit(ptr noundef %aClosureEnv, ptr noundef %sAllocator7, i32 noundef 104)
  %10 = load i32, ptr %iFlags.addr, align 4
  %11 = load ptr, ptr %pFunc.addr, align 8
  %iFlags9 = getelementptr inbounds nuw %struct.ph7_vm_func, ptr %11, i32 0, i32 5
  store i32 %10, ptr %iFlags9, align 8
  %12 = load ptr, ptr %pUserData.addr, align 8
  %13 = load ptr, ptr %pFunc.addr, align 8
  %pUserData10 = getelementptr inbounds nuw %struct.ph7_vm_func, ptr %13, i32 0, i32 7
  store ptr %12, ptr %pUserData10, align 8
  %14 = load ptr, ptr %zName.addr, align 8
  %15 = load ptr, ptr %pFunc.addr, align 8
  %sName = getelementptr inbounds nuw %struct.ph7_vm_func, ptr %15, i32 0, i32 2
  %zString = getelementptr inbounds nuw %struct.SyString, ptr %sName, i32 0, i32 0
  store ptr %14, ptr %zString, align 8
  %16 = load i32, ptr %nByte.addr, align 4
  %17 = load ptr, ptr %pFunc.addr, align 8
  %sName11 = getelementptr inbounds nuw %struct.ph7_vm_func, ptr %17, i32 0, i32 2
  %nByte12 = getelementptr inbounds nuw %struct.SyString, ptr %sName11, i32 0, i32 1
  store i32 %16, ptr %nByte12, align 8
  ret i32 0
}

attributes #0 = { nounwind uwtable "frame-pointer"="all" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }

!llvm.module.flags = !{!0, !1, !2, !3, !4}
!llvm.ident = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{i32 7, !"frame-pointer", i32 2}
!5 = !{!"clang version 23.0.0git (https://github.com/llvm/llvm-project 027447c61724385ae82f0a8dea34b45da2fd4c39)"}

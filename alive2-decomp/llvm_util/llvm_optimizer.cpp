...
optimize_module_with_pre(llvm::Module &M, string_view optArgs) {
  init_llvm_targets();
  ...
  // Register a one-shot BeforeNonSkippedPass callback that clones M after
  // runBeforePass has assigned !mymd tags but before any pass transforms it.
  PassInstrumentationCallbacks PIC;
  unique_ptr<llvm::Module> pre_snapshot;
  PIC.registerBeforeNonSkippedPassCallback(
      [&](StringRef, Any) {
        if (!pre_snapshot)
          pre_snapshot = CloneModule(M);
      });
